# -*- coding: utf-8 -*-
"""data_analysis_colab2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1muvbG6aNRfTrfl6gbUcOV2q6MS4TtXFI
...
# Documented exclusion: this participant's justifications showed clear evidence of consulting an external AI chatbot live during the study
# (e.g., copy-pasted dialogue like "give me the survey AI recommendation after this"), which is inconsistent with the study design of presenting
# a single predefined AI recommendation. All 7 of their task responses are excluded from analysis.
...
# Fix browser-translation artifacts: a few participants' browsers auto-translated the page before submission, capturing translated dropdown text instead of the original English value.
# Clear translations are recoded; "Tılımlı" has no standard meaning and is left as unknown rather than guessed.
...
translation_map_education = {
    '研究生学位': 'Postgraduate degree',
    'Lisans derecesi': 'Undergraduate degree',
    'Pós-graduação': 'Postgraduate degree',
    '大学卒業または同等の資格': 'College or equivalent',
}
...
df['ageGroup'] = df['ageGroup'].replace(translation_map_age)
df['educationLevel'] = df['educationLevel'].replace(translation_map_education)
...
print()
print("ageGroup after cleanup:")
print(df['ageGroup'].value_counts(dropna=False))
print()
print("educationLevel after cleanup:")
...
for v in df['ageGroup'].unique():
    print(repr(v), [hex(ord(c)) for c in v])
...
# Static study-design reference: which answer (0=A, 1=B, 2=C, 3=D) the AI recommends for each task, under each condition. This is fixed by the
# task design (from tasks.js) and is the same for every participant - it's not personal data, so it doesn't need to live in Firestore.
TASK_REFERENCE = {
...
# Sanity check: for the 'correct' condition, the AI-recommended answer should always equal correctAnswer.
...
if 'justificationScore' in df.columns:
    df = df.drop(columns=['justificationScore'])
...
print(df['aiUseFrequency'].unique())
...
condition_order = ['correct', 'incorrect', 'partial']
condition_labels = ['Correct', 'Incorrect', 'Partially correct']

followed_rate = df.groupby('aiCondition')['followedAI'].mean().reindex(condition_order) * 100

fig, ax = plt.subplots(figsize=(7, 5))
bars = ax.bar(condition_labels, followed_rate.values, color=['#55A868', '#C44E52', '#DD8452'])

ax.set_ylabel('Mean rate of following the AI recommendation (%)')
ax.set_ylim(0, 100)
ax.set_title('Rate of following the AI recommendation, by condition')

for bar, val in zip(bars, followed_rate.values):
    ax.text(bar.get_x() + bar.get_width()/2, val + 1.5, f'{val:.1f}%', ha='center', fontweight='bold')

plt.tight_layout()
plt.show()

print(followed_rate.round(1))
...
# r = 1 - (2*T) / (n*(n+1)), where T is the smaller rank sum (the statistic scipy's wilcoxon returns by default) and n is the number of non-zero differences in that pair.
